...
import { Stethoscope, ArrowLeft, ChevronRight, Sparkles } from 'lucide-react';
...
const ProviderAdminSelect = () => {
  ...
  const items = [
    { 
      label: 'VMP Clinic Portal', 
      tag: 'Recommended',
      icon: Stethoscope,
      desc: 'Full veterinary management platform: OPD schedule, live appointments, digital e-prescriptions, pet parent history & GST billing.', 
      path: '/clinic/dashboard', 
      btnText: 'Open VMP Clinic Portal ➔',
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fa] flex items-center justify-center p-4 md:p-8 font-sans text-stone-850">
      ...
      <div className="relative z-10 w-full max-w-5xl rounded-[2.5rem] border border-slate-200/50 bg-[#faf9f6]/95 backdrop-blur-md shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[640px] md:h-[680px]">
        ...
        {/* LEFT COLUMN: Workspace Selector Area (radial amber highlight gradient background) */}
          ...
          <div className="my-auto py-4 space-y-6">
            <div className="space-y-1">
              <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/25 text-[10px] uppercase font-bold text-amber-700 tracking-wider">
                <Sparkles className="w-3 h-3" /> Clinic Suite
              </div>
              <h2 className="text-3xl font-display font-extrabold text-stone-900 tracking-tight">Clinic Workspace</h2>
              <p className="text-stone-500 text-xs font-medium">Select your clinic operational workspace to manage appointments and records</p>
            </div>
              ...
              {items.map((item) => {
                ...
                return (
                  <div
                    ...
                    className="group relative rounded-3xl border border-stone-200/80 bg-white/60 p-6 text-left transition-all duration-300 hover:border-amber-400 hover:bg-white hover:-translate-y-0.5 hover:shadow-lg cursor-pointer flex flex-col justify-between space-y-4"
                  >
                    ...
                    <button className="w-full h-10 rounded-full bg-stone-100 group-hover:bg-amber-400 group-hover:text-stone-900 text-stone-600 font-bold text-xs transition-colors flex items-center justify-center gap-1">
                      {item.btnText} <ChevronRight className="w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5" />
                    </button>
                  </div>
                );
              })}
        ...
        <div className="hidden md:block w-[52%] p-3 h-full">
          ...
          >
            ...
            <div className="relative z-10 m-6 p-4 bg-white/75 backdrop-blur-md border border-white/25 rounded-2xl shadow-xl w-[90%] max-w-sm transition-all duration-300 hover:-translate-y-1">
              ...
              <h5 className="text-xs font-bold text-stone-850">Full Veterinary Management</h5>
              <p className="text-[10px] text-stone-500 leading-normal mt-0.5">Control OPD scheduler, consult logs, digital e-prescriptions, billing registers, and automated WhatsApp patient updates live.</p>
            </div>
          </div>
        </div>
      </div>
...
export default ProviderAdminSelect;